...
# Set up logging
log_file_path = os.path.join(os.getcwd(), 'agent_conversation.log')
...
def initiate_chat(job_id, job_questionnaire_id, resume, job_description, candidate_email, job_questionnaire):
    try:
        ...
        api_key = os.getenv('OPENAI_API_KEY', 'sk-WVMOT-ux9F3MYCBpJdCTqCwjPfyCQa-iGBwD7yJtkST3BlbkFJyDQSA4BntLqFwINg-xRxS3DAsAbTbvlH5Cs2iUr2EA')
        ...
        def ranking_tool(candidate_email, ranking, conversation, resume, job_description):
            try:
                ...
                if not all([candidate_email_safe, resume_safe, job_description_safe, conversation_safe]):
                    logging.error("Error: One or more payload fields could not be converted to JSON-safe format.")
                ...
                logging.info(f"Ranking entry saved with unique ID: {unique_id} for candidate email: {candidate_email_safe}")
                return f"Ranking entry saved with unique ID: {unique_id} for candidate email: {candidate_email_safe}"

            except Exception as e:
                logging.error(f"An error occurred in the ranking tool: {e}")
                return None
        ...
        logging.info(f"Group chat initiated for candidate: {candidate_email} with job ID: {job_id}")
        
    except Exception as e:
        logging.error(f"An error occurred in the initiate_chat function: {e}")
