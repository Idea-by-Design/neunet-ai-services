...
def fetch_candidates_with_github_links():
    try:
        query = "SELECT c.id, c.name, c.email, c.skills, c.experience FROM c WHERE IS_DEFINED(c.links.github)"
        candidates = list(resumes_container.query_items(query=query, enable_cross_partition_query=True))
        print(f"Fetched {len(candidates)} candidates with GitHub links.")
        return candidates
    except Exception as e:
